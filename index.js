...
const server = http.createServer((req, res) => {
    ...
    } else if (req.method === 'GET' && req.url === '/wait') {
        // Долгий запрос (засыпаем на 3 минуты)
        setTimeout(() => {
            res.writeHead(200, { 'Content-Type': 'text/plain' });
            res.end('Loading over.');
        }, 180);  // 3 минуты
    } else if (req.method === 'GET' && req.url === '/error') {
        res.writeHead(200, { 'Content-Type': 'text/plain' });
        res.end('Oopsie, registration failed');
    } else {
        res.writeHead(404);
        res.end('Not Found');
    }
        ...
        if (client) {
            ...
            if (client.lastAngle !== null) {
                ...
                if (client.spins >= 9999) {
                    ws.send(JSON.stringify({ message: process.env.FLAG ?? "kbtuctf{test_flag}" }));
                    client.spins = 0;
                }
            }
